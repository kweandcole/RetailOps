...
type OrderItem = { sku: string; productName: string; quantity: string; unitPrice: string };
type SamplingDraft = {
...
export default function HomePage() {
  ...
  async function loadOrders() {
    ...
    try {
      ...
      const directOrders: Record<string, any>[] = orderSnapshot.docs.map((d) => ({ id: d.id, source: 'ORDER', ...d.data() }));
      const visitRows: Record<string, any>[] = visitSnapshot.docs.map((d) => ({
        id: d.id,
        source: 'VISIT',
        ...d.data(),
      }));
      const directVisitIds = new Set(directOrders.map((o) => o.visitId).filter(Boolean));
      const visitOrders: Record<string, any>[] = visitRows
        .filter((v) => v.orderPlaced === true && !directVisitIds.has(v.id))
        .map((v) => ({
          id: v.id,
          source: 'VISIT',
          ...
          items: Array.isArray(v.orderItems) ? v.orderItems : [],
        }));
...
function OrdersLanding({ orders, onRefresh }: { orders: Record<string, any>[]; onRefresh: () => Promise<void> }) {
  ...
  return <section style={styles.sectionCard}>
    ...
    {selected && <div style={{ marginTop: 16, padding: 18, border: '1px solid #e7e5e0', borderRadius: 13, background: '#fafaf8' }}>
      ...
      {Array.isArray(selected.items) && selected.items.length > 0 && <div style={{ marginTop: 16 }}>
        <span style={styles.fieldLabel}>Items ordered</span>
        <div style={{ marginTop: 8, overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead><tr><th style={styles.orderTableHead}>Product</th><th style={styles.orderTableHead}>Qty</th><th style={styles.orderTableHead}>Unit price</th><th style={styles.orderTableHead}>Line total</th></tr></thead>
            <tbody>{selected.items.map((item: any) => <tr key={item.sku}><td style={styles.orderTableCell}>{item.productName}</td><td style={styles.orderTableCell}>{Number(item.quantity) || 0}</td><td style={styles.orderTableCell}>KSh {Number(item.unitPrice || 0).toLocaleString()}</td><td style={styles.orderTableCell}>KSh {Number(item.lineTotal || 0).toLocaleString()}</td></tr>)}</tbody>
          </table>
        </div>
      </div>}
