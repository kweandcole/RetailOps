...
type SamplingDraft = { conducted: boolean | null; customersSampled: string; bottlesSold: string; feedback: string; reason: string; };
...
export default function HomePage() {
  ...
  const [sampling, setSampling] = useState<SamplingDraft>({
    conducted: null, customersSampled: '', bottlesSold: '', feedback: '', reason: '',
  });  useEffect(() => {
    try {
  ...
  async function saveSampling() {
    ...
    const numericFields = [sampling.customersSampled, sampling.bottlesSold];
    if (sampling.conducted && numericFields.some((value) => value === '' || !Number.isInteger(Number(value)) || Number(value) < 0)) {
    ...
    try {
      await updateDoc(doc(getFirebaseDb(), 'visits', activeVisitId), {
        sampling: {
          ...
          bottlesSold: sampling.conducted ? Number(sampling.bottlesSold) : 0,
      ...
      <div style={styles.samplingCard}>
        ...
        {sampling.conducted === true && <>
          <div style={styles.samplingMetrics}>
            ...
            <label style={styles.stockField}><span style={styles.fieldLabel}>Bottles sold</span><input type="number" min="0" step="1" inputMode="numeric" value={sampling.bottlesSold} onChange={(e) => setSampling((current) => ({ ...current, bottlesSold: e.target.value }))} placeholder="0" style={styles.stockInput} /></label>
          </div>
