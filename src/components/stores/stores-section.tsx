...
export default function StoresSection({ onStartVisit }: { onStartVisit: (store: Store) => void }) {
  ...
  const attentionCount = (store: Store) => Number((store as any).stockAlerts || 0) + Number((store as any).expiryAlerts || 0);
  ...
  return (
    <section>
      ...
      <div style={styles.list}>
        {!loading && filteredStores.map((store) => <article key={store.outletId} style={{ ...styles.card, ...(attentionCount(store) > 0 ? styles.cardAttention : {}) }}>
          ...
          <div style={styles.attentionRow}>
            {Number((store as any).stockAlerts || 0) > 0 && <span style={styles.alertBadge}>Stock alert: {(store as any).stockAlerts}</span>}
            {Number((store as any).expiryAlerts || 0) > 0 && <span style={styles.expiryBadge}>Expiry: {(store as any).expiryAlerts}</span>}
            {attentionCount(store) === 0 && <span style={styles.clearBadge}>No immediate stock/expiry alerts</span>}
          </div>
